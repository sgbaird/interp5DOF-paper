...
\begin{equation}
    T(N_p^2L) \approx T_{\text{sym}}(N_p^2L)+T_\text{\text{pd}}(L^2) 
\end{equation}
where $T$, $T_{\text{sym}}$ and $T_{\text{pd}}$ are the runtimes of the symmetrization and pairwise-distance computations, respectively.
%In other words, even though the symmetrization step scales as $O(N_p^2L)$, while the pairwise distance step scales as $O_\text{pd}(L^2)$, and $N_p^2L \ll L^2$, the actual \emph{time} to compute the pairwise distance matrix is nearly negligible