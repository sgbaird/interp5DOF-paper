...
\zexternaldocument*{supp-frankenstein-1}
\input{values.tex}
\input{abbrev.tex}
...
\begin{abstract}
    We introduce the \gls{vfz} framework which is useful for \gls{gb} structure-property models and gaining insights about the nature of a \gls{5dof} \gls{fz} for both cubic and non-cubic symmetries and potentially alloys. We cover the methods associated with the \gls{vfz} framework. The \gls{vfz} framework offers an advantage over other \gls{5dof} based property interpolation methods because %it is constructed as a point set in a manifold. This means that
    ...
    The best performance was achieved with \gls{gpr} which results in a reduction of the \gls{rmse} by \calcnum{(\avgrmse-\gprrmse)/\avgrmse*100}\% relative to \gls{rmse} of a constant, average model. This error is comparable to the minimum expected uncertainty associated with reconstruction of noise-free experimental polycrystalline data. We then use \gls{gpr} to interpolate simulated bi-crystal datasets for Fe and Ni and demonstrate better than (\SI{34.4}{\percent} vs. \SI{21.2}{\percent} improvement) and similar (\SI{57.6}{\percent} vs. \SI{56.4}{\percent} improvement) performance to prior work, respectively. %Interpolation on a large, noisy, \gls{ms} Fe simulation dataset improves performance by \SI{34.4}{\percent} compared to \SI{21.2}{\percent} in prior work. %and we discover that
    ...
    The noise and non-uniform sampling in the Fe dataset make it difficult to resolve low \gls{gbe} (i.e. cusps) and to validate the model. %Interpolation on a small, low-noise \gls{ms} Ni simulation dataset is similar to interpolation results previously reported, and
    We resolve the Ni dataset cusps with high accuracy, but uncertainty may be high in regions far from the input data. The trade-offs between noise, dataset size, sampling scheme, and repeat measurements must be carefully managed. % Using \gls{gpr}, we estimate the correlation lengths of the Fe and Ni datasets to be $\sim$\SIlist{8.3;7.4}{\degree}, respectively, compared with a traditionally accepted value of $\sim$\SI{10}{\degree}. % (\SI{57.6}{\percent} vs. \SI{56.4}{\percent}).
    %Finally
    We provide a vectorized, parallelized, MATLAB interpolation function (\matlab{interp5DOF.m}) and related routines %in our \vfzorepo{} 
    (\url{github.com/sgbaird-5dof/interp}) which can be applied to future datasets for a variety of \gls{gb} properties and to better understand \gls{5dof} \glspl{fz}. For example, we estimate the maximum dimension of an $O_h$ \gls{vfz} to be $\sim$\SI{66.5}{\degree} in the \gls{gbo} sense. % which has potential for the creation of advanced \gls{gb} structure-property models for alloys and cubic/non-cubic crystallographic point groups. %The \gls{vfz} framework offers advantages for computing distances between \glspl{gb}, estimating property values for arbitrary \glspl{gb}, and modeling surrogates of computationally expensive \gls{5dof} functions and simulations.
\end{abstract}
...
%We present a new method for interpolating and predicting \gls{gb} properties from a set of measured/calculated values (e.g. \gls{gbe} from \gls{ms} simulations). We term our approach the \gls{vfz} framework. It is highly efficient and facilitates the use of large datasets to enhance prediction accuracy. We discuss motivation for (\cref{sec:intro:motivation}) and prior implementations of \gls{5dof} property prediction (\cref{sec:intro:prior}) and then highlight unique properties of the \gls{vfz} framework that offer advantages over other methods (\cref{sec:intro:vfzo}).
...
To calculate\footnote{\Gls{vfzgbo} Euclidean, hyperspherical arc length, and \gls{gbo} distances are computed via \vfzorepo{} function \matlab{GBdist4.m} which is used in the symmetrization function \matlab{get\_octpairs.m} and an example of ensemble \gls{vfzgbo} distance calculations is given in \matlab{plotting.m}.} the distance between a given \gls{gbo}, and the reference \gls{gbo}, we employ the standard 8D Euclidean distance
\begin{equation}
...
%Do I leave the 60 degree max dimension in the CMS paper?
Euclidean distances are an accurate approximation of arc length distances in a \gls{vfz} as shown in \cref{fig:dist-parity}. This also demonstrates that the maximum dimension of an $O_h$ \gls{vfz} is $\sim$\SI{66.5}{\degree} in the \gls{gbo} sense. %because the difference between the two metrics for the maximum pairwise distance ($pd_{max} \simeq \SI{60}{\degree}$) in a \gls{vfz} is small as shown in \cref{fig:dist-parity}.
...
Misorientation Convention & $q_m = q_A{q_B}^{-1}$ & $q_m = {q_A}^{-1}q_B$ \\
\bottomrule
\end{tabular}
...
%
%moved "generating random VFZOs" to supp
%
...
%update in excel first or replace with pgfmath commands
...
\gls{gpr}   & \glsxtrshort{vfz} & \glsxtrshort{brk} & \num{50000} & \num{\gprrmse{}} & \num{\avgrmse{}} & \absReduction{\gprrmse}{\avgrmse} & \percReduction{\gprrmse}{\avgrmse}   \\
Barycentric & \glsxtrshort{vfz} & \glsxtrshort{brk} & \num{50000} & \num{\baryrmse{}} & \num{\avgrmse{}} & \absReduction{\baryrmse}{\avgrmse} & \percReduction{\baryrmse}{\avgrmse} \\
\gls{idw}   & \glsxtrshort{vfz} & \glsxtrshort{brk} & \num{50000} & \num{\idwrmse{}} & \num{\avgrmse{}} & \absReduction{\idwrmse}{\avgrmse} & \percReduction{\idwrmse}{\avgrmse} \\
\gls{nn}    & \glsxtrshort{vfz} & \glsxtrshort{brk} & \num{50000} & \num{\nnrmse{}} & \num{\avgrmse{}} & \absReduction{\nnrmse}{\avgrmse} & \percReduction{\nnrmse}{\avgrmse} \\ \bottomrule
\end{tabular}
\end{table*}
...
\gls{gpr}   & \glsxtrshort{vfz} & \glsxtrshort{brk} & \num{50000} & \num{\gprmae{}} & \num{\avgmae{}} & \absReduction{\gprmae}{\avgmae}  & \percReduction{\gprmae}{\avgmae} \\
Barycentric & \glsxtrshort{vfz} & \glsxtrshort{brk} & \num{50000} & \num{\barymae{}} & \num{\avgmae{}} & \absReduction{\barymae}{\avgmae}  & \percReduction{\barymae}{\avgmae} \\
\gls{idw}   & \glsxtrshort{vfz} & \glsxtrshort{brk} & \num{50000} & \num{\idwmae{}} & \num{\avgmae{}} & \absReduction{\idwmae}{\avgmae}  & \percReduction{\idwmae}{\avgmae} \\
\gls{nn}    & \glsxtrshort{vfz} & \glsxtrshort{brk} & \num{50000} & \num{\nnmae{}} & \num{\avgmae{}} & \absReduction{\nnmae}{\avgmae} & \percReduction{\nnmae}{\avgmae} \\ \bottomrule
\end{tabular}
\end{table*}
...
It is worthwhile to note that both \gls{gpr} and \gls{idw} are kernel-based in that a model parameter controls the size of the region that can influence the interpolation results. In the \gls{gpr} case, this is automatically calculated via an internal fitting routine of \matlab{fitrgp()}. \gls{nn} distances can lead to insight about correlation lengths in a given \gls{vfzgbo} set and are used in the \gls{idw} implementation (future work). For \gls{idw}, the radius of influence is set to $r=\sqrt{2} \mu$, where $\mu$ is the mean \gls{nn} distance. It is likely that better tuning of the kernel parameters in these two methods (such as use of built-in hyperparameter optimization in the case of \matlab{fitrgp()}) could further decrease their interpolation errors. Additionally, for \gls{gpr}, use of the \matlab{'exact'} \matlab{PredictMethod} or a larger \gls{fic} (\matlab{'fic'} \matlab{PredictMethod}) set size will also likely reduce interpolation error.

By contrast, barycentric interpolation automatically adjusts its effective region of influence because the size of the simplices in the mesh decreases as the number of vertices increases. More uniformly distributed meshes (such as obtained via constrained optimization \cite{dolanBenchmarkingOptimizationSoftware2004,ConstrainedElectrostaticNonlinear2020}) will likely result in lower, more uniform interpolation error, especially for this simplex-based approach which can exhibit high-aspect ratio facets and non-intersections outside the bounds of the mesh \cite{bairdBarycentricInterpolation7SphereUnderReview}.  %(\cref{fig:high-aspect-non-int}).
...
\subsubsection{Experimental Uncertainty of Polycrystalline \glsentrytitlecase{gbe}{short} Reconstruction }
...
\label{sec:results:vis}
Visual illustrations of property predictions along a 1D arc and within a fixed low-\gls{gbe}/low-$\Sigma$ misorientation (varying \gls{bp} normal) are given in \cref{sec:results:vis:arc,sec:results:vis:mis}, respectively.

\subsubsection{Interpolation Along a 1D Arc} \label{sec:results:vis:arc}
%We present interpolation results plotted in a 1D arc in the full \gls{5dof} \gls{gb} space (\cref{sec:results:vis:arc}).
%
...
%     \end{subfigure}
%        \caption{\Glspl{gbe} along direct paths in a \gls{vfz} between the $\Sigma3$ \gls{ct} boundary and minimum \gls{gbe} (\subref*{fig:tunnel-3-5}) $\Sigma5$, (\subref*{fig:tunnel-3-7}) $\Sigma7$, (\subref*{fig:tunnel-3-9}) $\Sigma9$, and (\subref*{fig:tunnel-3-11}) $\Sigma11$ \glspl{gb} within the \citet{olmstedSurveyComputedGrain2009} dataset. \Gls{gbe} values are plotted for the \gls{brk} and \gls{gpr} models which both used \citet{olmstedSurveyComputedGrain2009} as \inpt{} data. }
...
\subsubsection{Low-$\Sigma$/Low-\glsxtrshort{gbe} Fixed Misorientation}
\label{sec:results:vis:mis}
The misorientation is fixed to the misorientation of the minimum \gls{gbe} \gls{ct} $\Sigma3$ [111] \gls{gb} from the Olmsted Ni dataset while the \gls{bp} normal is varied across the unit-sphere. \Gls{gbe} is sampled via a \gls{gpr} model trained on \num{50000} \inpt{} points (\cref{fig:ni-gpr-brk}a), and \glspl{gbe} sampled directly from the \gls{brk} validation function are also provided (\cref{fig:ni-gpr-brk}b). Some artifacts can be seen in the \gls{gpr} model which are likely due to distance overestimation (\cref{sec:methods:framework:vfz-dist}) and can be addressed via e.g. ensemble methods. For the \num{20000} points plotted in \cref{fig:ni-gpr-brk}, the \gls{rmse} is \SI{\nigprbrkrmse{}}{\J\per\square\m} which is larger than the overall model performance of \SI{\gprrmse{}}{\J\per\square\m} (\cref{tab:rmse-error-comparison}). This is consistent with an overall expectation that model performance worsens near sharp features or discontinuities in the model. Similarly, overestimation of the $\Sigma3$ cusp is present, consistent with poorer low \gls{gbe} predictions in \cref{fig:brkparity50000}. The overestimation also suggests that nearby values (outside of the 2D subspace shown) are large enough to shift the \gls{gpr} model in that region.

\begin{figure*}
    \centering
    \includegraphics{figures/ni-gpr-brk.png}
    \caption{\Gls{gbe} for various \gls{bp} normals using the fixed misorientation of the minimum \gls{gbe} \gls{ct} $\Sigma3$ [111] \gls{gb} for Ni are sampled (a) from a \gls{gpr} model trained on \num{50000} \gls{brk} \inpt{} points and (b) directly from the \gls{brk} validation function. }
    \label{fig:ni-gpr-brk}
\end{figure*}
...
In this work, we presented the \gls{vfz} framework for (i) computing distances between \glspl{gb} and (ii) predicting the properties of \glspl{gb} from existing measurements.
We found that distance calculations in the \gls{vfz} framework are dramatically more computationally efficient %($O(N_p^2L)$)
than traditional methods %($O(N_p^4L^2)$)
at the expense of infrequent, large distance overestimation which can be addressed through ensemble techniques at a small computational cost.
...
In future work, we believe it will be instructive to probe properties of a \gls{5dof} \gls{fz}. For example, we found that the maximum dimension of an $O_h$ \gls{vfz} is $\sim$\SI{66.5}{\degree} in the \gls{gbo} sense. Other topics of interest are correlation lengths for various \gls{gb} properties and crystallographic symmetries within a \gls{fz}, upper and lower bounds for metastable properties, and paths through \gls{5dof} space w.r.t. low-Sigma \glspl{gb} of interest.
...
Misorientation quaternions are given by\footnote{The inverse of this convention, $q_m = q_A{q_B}^{-1}$ is used in \cite{francisGeodesicOctonionMetric2019}}:
\begin{equation} \label{eq:mis-convention}
    q_m = {q_A}^{-1}q_B
\end{equation}
...
\begin{equation} \label{eq:quat-mult}
...
In this work, we use the convention that $P=1$ throughout the various operations in the \vfzorepo{} ($P \equiv $ \matlab{epsijk}). For some datasets, it may be necessary to flip the rotation convention as with the \citet{olmstedSurveyComputedGrain2009} dataset\footnote{e.g. by applying \matlab{oflip.m} once prior to interpolation}. We encourage interested readers to refer to \citet{rowenhorstConsistentRepresentationsConversions2015} to understand the redefined versions of quaternion multiplication, quaternion rotation, etc. \Gls{bp} unit normals are expressed pointing away from grain A and in the reference frame of grain A (i.e. the outward-pointing normal convention).
