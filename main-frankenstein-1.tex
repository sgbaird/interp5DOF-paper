...
The misorientation is fixed using the minimum \gls{ct} $\Sigma$3 \gls{gb} while the \gls{bp} normal is varied across the unit-sphere, for which \gls{gbe} is sampled via a \gls{gpr} model trained on \num{50000} \inpt{} points. \Glspl{gbe} sampled directly from the \gls{brk} validation function are also provided.
...
\section{Misorientation Convention}
\label{sec:app:convention}
Misorientation quaternions are given by\footnote{The inverse of this convention is used in \cite{francisGeodesicOctonionMetric2019}}:
\begin{equation}
    q_m = {q_A}^{-1}q_B
\end{equation}
