...
In this work, random \gls{vfzo} set sizes between \num{100} to \num{50000} \glspl{vfzo} correspond to resolutions of \SI{10.7175 \pm 0.3684}{\degree} and \SI{2.6479 \pm 0.2254}{\degree}, respectively, out of approximately \num{70} trial runs per \gls{vfzo} set size (\cref{fig:nndist-vs-setsize}). For a specific \num{50000} \gls{vfzo} set, the \gls{nn} $\omega$ distance is \SI{2.87 \pm 0.7}{\degree} (\cref{fig:nnhist-knn-50000}a) while the average 100th \gls{nn} distance is within \SI{10}{\degree} (\cref{fig:nnhist-knn-50000}b). This indicates that many points fall within a typical \gls{gb} correlation length (\SI{10}{degree} \cite{olmstedSurveyComputedGrain2009}) for a given \outpt{} point in large set sizes. 
...
The authors would like to thank thank Ian Chesser, Toby Francis, Victoria Baird, and Brandon Snow for useful discussions. The material presented here is based upon work supported by the National Science Foundation under Grant No. 1610077.
...
\begin{itemize}
    \item \cref{fig:bary-remove-deg} illustrates the importance that a symmetrized \gls{gbo} set in the 8D Cartesian (7-sphere) \gls{vfz} is analogous to a geodesic \textit{arc} on the 2-sphere (\cref{fig:bary-remove-deg}b) with a degenerate dimension which is then removed by the first \gls{svd} transformation (\cref{fig:bary-remove-deg}b). This would otherwise be difficult to visualize in a 4D Cartesian (3-sphere) to 3D Cartesian (2-sphere) case.
...
\outpt{} points, using the \texttt{interp5DOF} sub-routine \texttt{proj\_down} (which depends on MATLAB's built-in \gls{svd} implementation, \texttt{svd()}), performing the transformation, and splitting the points (reverse of concatenation step).
