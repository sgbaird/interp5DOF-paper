...
\subsubsection{Triangulation of the \glsentrytitlecase{vfz}{long}}
...
Using a separate homeomorphic rigid \gls{svd} transformation from the triangulation, the original mesh and prediction \glspl{gbo} are simultaneously rotated and translated from 8D Cartesian to 7D Cartesian. Because our \gls{svd} approach is homeomorphic, the triangulation calculated in \cref{sec:methods:tri} is then superimposed onto the newly transformed input points resulting in a 7D Cartesian \gls{vfz}.
...
.
%Because octonion distances are well-approximated by Euclidean distances in the \gls{vfzo} framework and both distance metrics
